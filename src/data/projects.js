...
//   'app'     — a mobile app live on a public store. CTA goes to the listing.
//   'beta'    — a mobile app still in closed testing, so there is no public
//               listing to link to. CTA asks for early access.
//
...
const make = (p) => {
  ...
  return {
    ...
    // Store apps have no host to fetch an icon from, but their app icon is
    // copied into the manifest by hand, so read the manifest first either way.
    favicon: faviconManifest.clients[p.id] ?? (host ? fav(p.id, host) : null),
...
export const addressFor = (p) => {
  ...
  if (p.kind === 'app') return `${p.store ?? 'Google Play'} · ${p.pkg}`
  if (p.kind === 'beta') return `${p.pkg} · closed testing`
  return p.host
}
...
export const projects = [
  ...
  make({
    ...
    year: '2023',
    summary:
      'Headless commerce on Shopify + custom CMS — editorial category merchandising, faceted collection filtering with sort and switchable grid density, multi‑currency storefront and a checkout tuned for conversion.',
  ...
  make({
    ...
    year: '2024',
    summary:
      'Brand-led site for a 100-strong marketing agency — scroll-driven motion, long-form campaign case studies with outcome metrics, and careers and lead capture throughout.',
  ...
  make({
    ...
    year: '2026',
    summary:
      'Mumbai interior design studio — an editorial project gallery of completed homes tagged by room and location, plus turnkey interiors, modular kitchens, civil work and 3D visualisation.',
  ...
  make({
    ...
    year: '2024',
    summary:
      'Global NGO platform — multi‑currency donation flow with one‑time and recurring giving, live campaign progress against goal, and a project directory filterable by cause, country and partner.',
    stack: ['React', 'CMS', 'GSAP', 'Payments'],
  ...
  make({
    ...
    year: '2026',
    summary:
      'Islamic companion app, live on Google Play in 176 countries — prayer times with a background azan engine, full Quran in page-mode mushaf, qibla, duas and a daily-deeds tracker.',
    stack: ['React Native', 'Expo', 'Zustand', 'Node', 'MongoDB'],
    url: 'https://play.google.com/store/apps/details?id=com.ashshifa.app',
    accent: 'from-emerald-300/25 via-amber-200/10 to-transparent'
  }),
  make({
    id: 'plusveda',
    kind: 'beta',
    title: 'PlusVeda',
    domain: 'Pharmacy SaaS',
    pkg: 'com.medstock.app',
    year: '2026',
    summary:
      'Pharmacy stock, billing and finance platform — bill scanning to receive stock, FEFO batch picking so expired stock is never sold, expiry value‑at‑risk, multi‑warehouse transfers, supplier orders, post‑dated cheques and GST invoicing. Ships as an Android app and a Windows desktop build.',
    stack: ['React Native', 'Expo', 'Electron', 'Node', 'MongoDB'],
    accent: 'from-teal-300/25 via-emerald-400/10 to-transparent'
  }),
  make({
    id: 'bakerandco',
  ...
  make({
    id: 'autopart',
    title: 'Zakir Auto Works',
    domain: 'Automotive',
    host: 'autodoorspecialist.com',
    year: '2025',
    summary:
      'Car door, power window, sunroof and central‑locking specialists in Mumbai — service catalogue, work gallery and instant WhatsApp quote capture.',
    stack: ['React', 'Vite', 'Tailwind', 'Vercel'],
    url: 'https://www.autodoorspecialist.com/',
    accent: 'from-amber-300/25 via-orange-400/10 to-transparent'
  }),
  make({
    id: 'zaidelectronics',
